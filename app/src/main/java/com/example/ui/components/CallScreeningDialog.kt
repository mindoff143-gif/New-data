...
fun CallScreeningDialog(
    ...
    onReceiveCallDirectly: ((number: String, name: String?) -> Unit)? = null
) {
    ...
    Dialog(onDismissRequest = onDismiss) {
        Card(
            ...
            Column(
                ...
                // Receive Call Directly Button (User Answers)
                Button(
                    onClick = {
                        onReceiveCallDirectly?.invoke(phoneNumber, callerName.ifBlank { null })
                        onDismiss()
                    },
                    modifier = Modifier
                        .fillMaxWidth()
                        .height(48.dp)
                        .testTag("receive_call_from_dialog_button"),
                    shape = RoundedCornerShape(14.dp),
                    ...
                    Text(
                        text = if (preferredLanguage == "hi") "कॉल उठाएं व बात करें (Receive Call)" else "Answer Call Personally",
                        fontWeight = FontWeight.Bold,
                        color = Color.White
                    )
                }
                ...
                OutlinedButton(
                    ...
                    Spacer(modifier = Modifier.width(8.dp))
                    Text(
                        text = if (preferredLanguage == "hi") "AI कॉल स्क्रीनिंग टेस्ट (Screen with AI)" else "Screen with AI",
                        fontWeight = FontWeight.Bold,
                        color = IndigoPrimary
                    )
                }
