...
fun MainAppScreen(viewModel: AssistantViewModel) {
    ...
    val voiceOutputAllowed by viewModel.voiceOutputAllowed.collectAsState()
        ...
        Box(
            ...
            when (selectedNavTab) {
                0 -> AssistantChatScreen(
                    ...
                    voiceOutputAllowed = voiceOutputAllowed,
                    onToggleVoiceOutput = { viewModel.toggleVoiceOutput() },
                    onStartListening = { viewModel.startListening() },
                    onStopListening = { viewModel.stopListening() },
                    onSendQuery = { query -> viewModel.sendUserQuery(query) },
                    onSendQueryWithAttachment = { text, uri, name, mime, type, size ->
                        viewModel.sendUserQueryWithAttachment(text, uri, name, mime, type, size)
                    },
                    onSpeakText = { text -> viewModel.speakText(text) },
                    onStopSpeech = { viewModel.stopSpeaking() },
                    onOpenInviteDialog = { showInviteDialog = true },
                    onTriggerIncomingCall = { name, number ->
                        viewModel.triggerIncomingCall(name, number)
                    }
                )
                ...
                4 -> CallsScreen(
                    ...
                    onTriggerReceiveCall = { name, number ->
                        viewModel.triggerIncomingCall(name, number)
                    }
                )
                ...
                5 -> SettingsScreen(
                    ...
                    onInviteFriendsClick = { showInviteDialog = true }
                )
            }
        }
    }
    ...
    if (showCallScreeningDialog) {
        CallScreeningDialog(
            ...
            onReceiveCallDirectly = { number, name ->
                viewModel.triggerIncomingCall(name ?: "Caller", number)
                viewModel.receiveIncomingCall()
            }
        )
    }

    // Active Incoming Call / In-Call Overlay
    activeCall?.let { call ->
        IncomingCallOverlay(
            callState = call,
            onReceiveCall = { viewModel.receiveIncomingCall() },
            onDeclineCall = { viewModel.declineIncomingCall() },
            onScreenCall = { viewModel.simulateCallScreening(call.callerNumber, call.callerName) },
